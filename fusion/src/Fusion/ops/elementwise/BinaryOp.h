...
DenseTensor<T> apply_binary_op(const DenseTensor<T>& lhs, const DenseTensor<T>& rhs) {
   ...
   execution::cpu::binary_elementwise<T, OpTag>(out.get_ptr(), lhs.get_ptr(), rhs.get_ptr(), ctx);
   return out;
}
} // namespace fusion::ops
