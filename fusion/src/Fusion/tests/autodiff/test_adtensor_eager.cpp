...
TEST(ADTensorEagerTest,
   ...
   EXPECT_TENSOR_EQ(y.raw(), test_builders::raw_sqrt_expected());
...
TEST(ADTensorEagerTest,
   ...
   EXPECT_FALSE(AutodiffContext<float>::has());
   EXPECT_TENSOR_EQ(z.raw(), test_builders::raw_square_plus_ones_expected());
...
TEST(ADTensorEagerTest,
   ...
   EXPECT_FALSE(AutodiffContext<float>::has());
   EXPECT_TENSOR_EQ(y.raw(), test_builders::raw_sum_axis1_expected());
   EXPECT_FALSE(y.requires_grad());
   EXPECT_FALSE(y.has_vid());
}
...
TEST(ADTensorEagerTest,
   ...
   EXPECT_FALSE(AutodiffContext<float>::has());
   EXPECT_TENSOR_EQ(z.raw(), test_builders::raw_linear_times_two_expected());
   EXPECT_FALSE(z.requires_grad());
   EXPECT_FALSE(z.has_vid());
}
