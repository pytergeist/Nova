...
namespace simd {
    ...
    inline void pow_f32_neon(float *dst, const float *a, const float *b, std::size_t n) {
        std::size_t i = 0;
        ...
        for (; i < n; ++i) dst[i] = std::pow(a[i], b[i]);
    }


    inline void maximum_f32_neon(float *dst, const float *a, const float *b, std::size_t n) {
        std::size_t i = 0;
        ...
        for (; i + 16 <= n; i += 16) {
            ...
            float32x4_t mask0 = vcgtq_f32(a0, b0);
            vst1q_f32(dst + i + 0, vbslq_f32(mask0, a0, b0));
            float32x4_t mask1 = vcgtq_f32(a1, b1);
            vst1q_f32(dst + i + 4, vbslq_f32(mask1, a1, b1));
            float32x4_t mask2 = vcgtq_f32(a2, b2);
            vst1q_f32(dst + i + 8, vbslq_f32(mask2, a2, b2));
            float32x4_t mask3 = vcgtq_f32(a3, b3);
            vst1q_f32(dst + i + 12, vbslq_f32(mask3, a2, b2));
        }
        for (; i + 4 <= n; i += 4) {
            float32x4_t va = vld1q_f32(a + i);
            float32x4_t vb = vld1q_f32(b + i);
            float32x4_t mask_va = vcgtq_f32(va, vb);
            vst1q_f32(dst + i, vbslq_f32(mask_va, va, vb));
        }
    ...
    inline void pow_f32_neon_scalar_rhs(float *dst, const float *a, const float b, std::size_t n) {
        ...
        for (; i < n; ++i) dst[i] = std::pow(a[i], b);
    }


    inline void maximum_f32_neon_scalar_rhs(float *dst, const float *a, float b, std::size_t n) {
        float32x4_t vb = vdupq_n_f32(b);
        std::size_t i = 0;
        for (; i + 16 <= n; i += 16) {
            float32x4_t a0 = vld1q_f32(a + i + 0);
            ...
            float32x4_t a2 = vld1q_f32(a + i + 8);
            float32x4_t a3 = vld1q_f32(a + i + 12);
            float32x4_t mask0 = vcgtq_f32(a0, vb);
            vst1q_f32(dst + i + 0, vbslq_f32(mask0, a0, vb));
            float32x4_t mask1 = vcgtq_f32(a1, vb);
            vst1q_f32(dst + i + 4, vbslq_f32(mask1, a1, vb));
            float32x4_t mask2 = vcgtq_f32(a2, vb);
            vst1q_f32(dst + i + 8, vbslq_f32(mask2, a2, vb));
            float32x4_t mask3 = vcgtq_f32(a3, vb);
            vst1q_f32(dst + i + 12, vbslq_f32(mask3, a2, vb));
        }
        for (; i + 4 <= n; i += 4) {
            float32x4_t va = vld1q_f32(a + i);
            float32x4_t mask_va = vcgtq_f32(va, vb);
    ...
    inline void pow_f32_neon(float *dst, const float *a, const float *b, std::size_t n) {
        for (std::size_t i = 0; i < n; ++i) dst[i] = std::pow(a[i], b[i]);
    }

    inline void maximum_f32_neon(float *dst, const float *a, const float *b, std::size_t n) {
        for (std::size_t i = 0; i < n; ++i) dst[i] = a[i] > b[i] ? a[i]: b[i];
    }
    ...
    inline void pow_f32_neon_scalar_rhs(float *dst, const float *a, float b, std::size_t n) {
        for (std::size_t i = 0; i < n; ++i) dst[i] = std::pow(a[i], b);
    }
    ...
    inline void maximum_f32_neon_scalar_rhs(float *dst, const float *a, const float b, std::size_t n) {
        for (std::size_t i = 0; i < n; ++i) dst[i] = a[i] > b ? a[i]: b;
    }
