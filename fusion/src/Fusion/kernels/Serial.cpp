...
std::vector<size_t> linear_to_coord(size_t idx, size_t stride1, size_t stride2, size_t dim1, size_t dim2) {
...
std::vector<size_t> unravel_idx(size_t idx, std::vector<size_t>& strides, std::vector<size_t>& shape) {
...
size_t ravel_idx(const std::vector<size_t>& coord,
    ...
    size_t idx = 0;
    for (size_t k = 0; k < coord.size(); k++) idx += coord[k] * strides[k];
    return idx;
}

size_t coord_to_linear(std::vector<size_t>& coords, std::vector<size_t>& strides, size_t axis1, size_t axis2) {
...
std::vector<float> swapaxes(std::vector<float>& a, std::vector<size_t>& shape,  int a1, int a2) {
    ...
    size_t size = std::accumulate(shape.begin(), shape.end(), size_t{1}, std::multiplies<int>());
    ...
    std::vector<float> diagonal2D(std::vector<float>& a, std::vector<size_t>& shape) {
