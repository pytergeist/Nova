...
inline Level runtime_level() {
  static Level cached = [] {
    ...
    if (!e) return static_cast<Level>(FUSION_LOG_COMPILED_LEVEL);
    int v = std::atoi(e);
...
inline void log_line(Level L, const char* file, int line, Args&&... args) {
  ...
  if (static_cast<int>(L) > FUSION_LOG_COMPILED_LEVEL) return;
  // If runtime level is lower, drop at runtime:
  if (static_cast<int>(L) > static_cast<int>(runtime_level())) return;
...
#define FUSION_LOGE(...) ::fusionlog::log_line(::fusionlog::Level::kError, __FILE__, __LINE__, __VA_ARGS__)
#define FUSION_LOGW(...) ::fusionlog::log_line(::fusionlog::Level::kWarn,  __FILE__, __LINE__, __VA_ARGS__)
#define FUSION_LOGI(...) ::fusionlog::log_line(::fusionlog::Level::kInfo,  __FILE__, __LINE__, __VA_ARGS__)
#define FUSION_LOGD(...) ::fusionlog::log_line(::fusionlog::Level::kDebug, __FILE__, __LINE__, __VA_ARGS__)
