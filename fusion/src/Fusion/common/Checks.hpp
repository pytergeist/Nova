...
inline std::string
make_msg(std::string_view head, std::string_view body,
...
inline void
bounds_check(long long idx, long long size,
             std::source_location loc = std::source_location::current()) {
   if (idx < 0 || idx >= size) {
      std::ostringstream oss;
      oss << "Index " << idx << " out of bounds for size " << size;
      throw std::out_of_range(
          make_msg("FUSION_BOUNDS_CHECK failed", oss.str(), loc));
   }
}
...
template <class T1, class T2,
          std::enable_if_t<has_size_and_ndims_t<T1>::value &&
                               has_size_and_ndims_t<T2>::value,
                           int> = 0>
inline void allow_scalar_binary(
   ...
   if (!(is_scalar1 || is_scalar2 || (sz1 == sz2))) {
      std::ostringstream oss;
      oss << "Elementwise op requires scalars or equal element counts; got "
             "size("
          << sz1 << ") vs size(" << sz2 << ")";
      throw std::runtime_error(
          make_msg("FUSION_ALLOW_SCALAR_BINARY failed", oss.str(), loc));
   }
}
