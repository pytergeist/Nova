...
inline void walk(int dim, const int inn, const BroadcastPlan &plan, std::array<uint8_t *, 3>& ptr, InnerFn &&inner) {
      if (dim == inn) {
...
inline void for_each_outer_then_inner_new(const BroadcastPlan &plan,
...
void binary_ewise_tag(const TensorT &A, const TensorT &B,
   ...
   FUSION_CHECK(A.is_initialised() && B.is_initialised(),
                "uninitialised tensor");
   //  FUSION_CHECK(A.size() == B.size(), "size mismatch in binary op"); //
   //  TODO: this only works for same lenght ops
