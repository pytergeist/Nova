 ...
 *   - If both tensors have the same size, it applies op to each pair of corresponding elements.
 *   - If the left tensor is a scalar (size 1), it applies op between its single value and each element of the right tensor.
 *   - If the right tensor is a scalar (size 1), it applies op between each element of the left tensor and that single value.
...
template <typename T>
Tensor<T>::Tensor(const T &value) : arr(1, value) {}
